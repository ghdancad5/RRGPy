...
# Static data for stock information
...
# Use static data instead of yfinance calls
tickers_to_show = tickers_metadata_dict['symbol']
...
for i in range(len(tickers)):
    ...
    rsr_tickers.append((100 + (rs_tickers[i] - rs_tickers[i].rolling(window=window).mean()) / rs_tickers[i].rolling(
        window=window).std(ddof=0)).dropna())
    rsr_roc_tickers.append(100 * ((rsr_tickers[i] / rsr_tickers[i][1]) - 1))
    rsm_tickers.append((101 + (
                (rsr_roc_tickers[i] - rsr_roc_tickers[i].rolling(window=window).mean()) / rsr_roc_tickers[i].rolling(
            window=window).std(ddof=0))).dropna())
...
def update_rrg():
    ...
    for i in range(len(tickers)):
        ...
        rsr_roc_tickers.append(100 * ((rsr_tickers[i] / rsr_tickers[i][1]) - 1))
        rsm_tickers.append((101 + (
                    (rsr_roc_tickers[i] - rsr_roc_tickers[i].rolling(window=window).mean()) / rsr_roc_tickers[
                i].rolling(window=window).std(ddof=0))).dropna())
        rsr_tickers[i] = rsr_tickers[i][rsr_tickers[i].index.isin(rsm_tickers[i].index)]
        rsm_tickers[i] = rsm_tickers[i][rsm_tickers[i].index.isin(rsr_tickers[i].index)]
...
for i in range(len(tickers_to_show)):
    ...
    entry = tk.Entry(table, textvariable=symbol_var, relief=tk.RIDGE, width=20, bg=bg_color, fg=fg_color,
                     font=('Arial', 12))
    entry.grid(row=i + 1, column=0)
    entry.bind('<Return>', update_entry)
    tk.Label(table, text=name, relief=tk.RIDGE, width=40, bg=bg_color, fg=fg_color, font=('Arial', 12)).grid(row=i + 1,
                                                                                                             column=1)
    tk.Label(table, text=price, relief=tk.RIDGE, width=20, bg=bg_color, fg=fg_color, font=('Arial', 12)).grid(row=i + 1,
                                                                                                              column=2)
    tk.Label(table, text=chg, relief=tk.RIDGE, width=20, bg=bg_color, fg=fg_color, font=('Arial', 12)).grid(row=i + 1,
                                                                                                            column=3)
...
def animate(i):
    ...
    for j in range(len(tickers)):
        ...
        if tickers[j] not in tickers_to_show:
            ...
            scatter_plots[j] = ax[0].scatter(filtered_rsr_tickers.values, filtered_rsm_tickers.values, color=color,
                                             s=marker_size)
            # Update the line
            line_plots[j] = \
            ax[0].plot(filtered_rsr_tickers.values, filtered_rsm_tickers.values, color='black', alpha=0.2)[0]
            ...
            annotations[j] = ax[0].annotate(tickers[j],
...
for j in range(len(tickers)):
    if tickers[j] in tickers_to_show:
        filtered_rsr_tickers = rsr_tickers[j].loc[
            (rsr_tickers[j].index > start_date) & (rsr_tickers[j].index <= end_date)]
        filtered_rsm_tickers = rsm_tickers[j].loc[
            (rsm_tickers[j].index > start_date) & (rsm_tickers[j].index <= end_date)]
        ...
        result_df = result_df.append({
            'JDK RS Momentum': filtered_rsm_tickers.values[-1],
            'JDK RS Ratio': filtered_rsr_tickers.values[-1],
            'Ticker': tickers[j],
            'Price': round(tickers_data[tickers[j]][end_date], 2),
            'Change': round(
                (tickers_data[tickers[j]][end_date] - tickers_data[tickers[j]][start_date]) /
                tickers_data[tickers[j]][start_date] * 100, 1)
        }, ignore_index=True)

# Save the DataFrame to a CSV file
result_df.to_csv('RRGPy_results.csv', index=False)
